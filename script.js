...
.neumorphic-menu.show ~ .app::after {
  content: '';
  position: fixed;
  top: 0;
  left: 0;
...
.delete-last-expense-btn:hover {
  opacity: 1;
  transform: scale(1.1);
  color: var(--danger);
}

.delete-last-expense-btn:active {
...
/* Обновленные стили для мини-графиков */
...
@media (max-width: 767px) {
  ...
  /* Убедимся, что контейнеры не выходят за пределы экрана */
  .financial-menu {
    width: 95%;
    max-width: 100%;
    padding: 10px;
...
/* Горизонтальная прокрутка для планшетов и десктопов */
@media (min-width: 768px) {
  ...
  .mini-charts-container::-webkit-scrollbar {
    height: 6px;
  }
  
  .mini-charts-container::-webkit-scrollbar-track {
    background: rgba(0,0,0,0.05);
    border-radius: 3px;
  }
  
  .mini-charts-container::-webkit-scrollbar-thumb {
    background: var(--primary);
    border-radius: 3px;
  }
}
...
/* Темная тема */
body.dark .mini-chart-widget {
  background: var(--bg);
  box-shadow: 3px 3px 6px var(--shadow-dark), 
              -3px -3px 6px var(--shadow-light);
}

body.dark .mini-chart-widget h4 {
  color: var(--primary);
}
