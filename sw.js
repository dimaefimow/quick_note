...
const CACHE_VERSION = 'finance-app-v3';
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      // Кешируем каждый файл app shell по отдельности: если один файл
      // недоступен (404/сеть), это не должно мешать закешировать остальные.
      ...
      return Promise.all([...shellPromises, ...externalPromises]);
    })
  );
  self.skipWaiting();
});
